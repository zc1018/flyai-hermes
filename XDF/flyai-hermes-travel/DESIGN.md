...
- Evidence reviewed: `README.md`, `static/index.html`, `static/app.js`, `static/styles.css`, `static/admin.html`, `static/admin.js`, `app/main.py`, `app/models.py`, Vercel React best-practices checklist
...
## Product goals
- Goals: 降低提问门槛，让用户快速描述旅行需求；把实时查询结果转成可扫读卡片；让配额/等待/失败状态可理解；让新用户不用阅读说明也能从示例和清单开始。
...
- New/changed components: scene prompt cards、travel brief checklist、empty state action buttons、query stats row、friendly progress shell、route overview、segment price pill、history search。
...
## Interaction states
- Loading: 查询按钮禁用，结果区显示进度卡，可停止等待。
- Empty: 登录后显示可操作的空状态和示例查询，示例会直接填入输入框。
